...
contract BounceLottery is BounceBase, Random {
    ...
    function createV2(
        ...
        checkCreator(keccak256(abi.encode(id, PoolType.Random)), expireAt, signature);
    ...
    function betPermit(
        ...
        checkUser(keccak256(abi.encode(index, PoolType.Random)), expireAt, signature);
        _bet(index);
    }
    ...
    function userClaim(uint256 index) external nonReentrant isPoolExist(index) isClaimReady(index) {
        ...
        if (isWinner(index, msg.sender)) {
            uint256 amount0 = pool.amountTotal0.div(pools[index].nShare);
            uint256 releaseAmount = computeReleasableAmount(index, amount0);
            if (releaseAmount > 0) {
                ...
                if (actualReleaseAmount > 0) {
                    ...
                    if (myReleased[msg.sender][index] == amount0) {
                        myClaimed[msg.sender][index] = true;
                    }
                    emit UserClaimed(index, msg.sender, actualReleaseAmount, 0);
                }
            }
        } else {
            uint256 amount1 = pool.amount1PerWallet;
            if (pool.token1 == address(0)) {
            ...
            myClaimed[msg.sender][index] = true;
            emit UserClaimed(index, msg.sender, 0, amount1);
        }
    }
