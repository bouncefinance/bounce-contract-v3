...
const deployFunction: DeployFunction = async function ({
    ...
    console.log('Running BounceRandomSelectionNFT deploy script')
    ...
    if (chainId === CHAINID.ZKSYNC || chainId === CHAINID.ZKSYNC_TEST) {
        await deployProxy(deployments, deployer, 'BounceRandomSelectionNFT', [
            txFeeRatio,
            stakeContract,
            signer,
            vrfCoordinator,
    ...
    const { address } = await deploy('BounceRandomSelectionNFT', {
        from: deployer,
        log: true,
    ...
    console.log('BounceRandomSelectionNFT deployed at', address)
}
...
deployFunction.tags = ['BounceRandomSelectionNFT']
