...
const deployFunction: DeployFunction = async function ({
    ...
    console.log('Running BounceFixedSwapNFT deploy script')
    ...
    if (chainId === CHAINID.ZKSYNC || chainId === CHAINID.ZKSYNC_TEST) {
        await deployProxy(deployments, deployer, 'BounceFixedSwapNFT', [txFeeRatio, stakeContract, signer])
        return
    }

    const { address } = await deploy('BounceFixedSwapNFT', {
        from: deployer,
        log: true,
    ...
    console.log('BounceFixedSwapNFT deployed at', address)
}
...
deployFunction.tags = ['BounceFixedSwapNFT']
