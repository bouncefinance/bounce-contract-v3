...
const deployFunction: DeployFunction = async function ({
    ...
    console.log('Running BounceFixedSwap deploy script')
    ...
    if (chainId === CHAINID.ZKSYNC || chainId === CHAINID.ZKSYNC_TEST) {
        await deployProxy(deployments, deployer, 'BounceFixedSwap', [txFeeRatio, stakeContract, signer])
        return
    }

    const { address } = await deploy('BounceFixedSwap', {
        from: deployer,
        log: true,
    ...
    console.log('BounceFixedSwap deployed at', address)
}
...
deployFunction.tags = ['BounceFixedSwap']
